...
const LOCAL_UNSAVED_EDITOR_KEY = 'strategisk-unsaved-editor-draft'
...
const pendingCachedFileMeta = (cacheId,file,category='thesis') => ({id:uid(),driveFileId:'',localCacheId:cacheId,name:file.name||'file',mimeType:file.type||'application/octet-stream',category,pending:true})

const MEDIA_CACHE_DB = 'strategisk-media-cache-v1'
const MEDIA_CACHE_STORE = 'media'
const MEDIA_CACHE_FRESH_MS = 7 * 24 * 60 * 60 * 1000
const MEDIA_FETCH_LIMIT = 3
const mediaMemoryUrls = new Map()
let mediaFetchActive = 0
const mediaFetchQueue = []
const openMediaCache = () => new Promise((resolve,reject)=>{
  if(!('indexedDB' in window)) return resolve(null)
  const req=indexedDB.open(MEDIA_CACHE_DB,1)
  req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains(MEDIA_CACHE_STORE))db.createObjectStore(MEDIA_CACHE_STORE,{keyPath:'id'})}
  req.onsuccess=()=>resolve(req.result)
  req.onerror=()=>reject(req.error)
})
async function getMediaCache(id){
  if(!id)return null
  try{const db=await openMediaCache();if(!db)return null;return await new Promise((resolve,reject)=>{const tx=db.transaction(MEDIA_CACHE_STORE,'readonly'),req=tx.objectStore(MEDIA_CACHE_STORE).get(id);req.onsuccess=()=>resolve(req.result||null);req.onerror=()=>reject(req.error)})}catch{return null}
}
async function putMediaCache(id,blob,meta={}){
  if(!id||!blob)return
  try{const db=await openMediaCache();if(!db)return;await new Promise((resolve,reject)=>{const tx=db.transaction(MEDIA_CACHE_STORE,'readwrite');tx.objectStore(MEDIA_CACHE_STORE).put({id,blob,mimeType:blob.type||meta.mimeType||'',name:meta.name||'',savedAt:Date.now()});tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)})}catch{}
}
async function clearMediaCache(){
  try{for(const u of mediaMemoryUrls.values())URL.revokeObjectURL(u);mediaMemoryUrls.clear();const db=await openMediaCache();if(!db)return;await new Promise((resolve,reject)=>{const tx=db.transaction(MEDIA_CACHE_STORE,'readwrite');tx.objectStore(MEDIA_CACHE_STORE).clear();tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)})}catch{}
}
const sleep = ms => new Promise(r=>setTimeout(r,ms))
const queueMediaFetch = task => new Promise((resolve,reject)=>{
  const run=async()=>{mediaFetchActive++;try{resolve(await task())}catch(e){reject(e)}finally{mediaFetchActive--;const next=mediaFetchQueue.shift();if(next)next()}}
  if(mediaFetchActive<MEDIA_FETCH_LIMIT)run();else mediaFetchQueue.push(run)
})
async function fetchDriveMediaBlob(fileId,token,{retries=true}={}){
  if(!fileId||!token)return null
  return queueMediaFetch(async()=>{
    const waits=retries?[0,900,2500,6000]:[0]
...
function useNearViewport(rootMargin='500px'){
  const ref=useRef(null),[near,setNear]=useState(false)
  useEffect(()=>{
    const el=ref.current
    if(!el){setNear(true);return}
    if(!('IntersectionObserver' in window)){setNear(true);return}
    const io=new IntersectionObserver(entries=>{if(entries.some(e=>e.isIntersecting)){setNear(true);io.disconnect()}},{rootMargin})
    io.observe(el)
    return()=>io.disconnect()
  },[rootMargin])
  return [ref,near]
}
function useMediaSrc(media,token,enabled=true){
  const id=typeof media==='object'?media?.driveFileId:''
  const initial=typeof media==='string'?media:(media?.dataUrl||mediaMemoryUrls.get(id)||'')
  const [src,setSrc]=useState(initial)
  const [loading,setLoading]=useState(false)
  useEffect(()=>{
    let cancelled=false
    const setBlob=(blob,cacheId=id)=>{
      if(cancelled||!blob)return
      let u=cacheId?mediaMemoryUrls.get(cacheId):''
      if(!u){u=URL.createObjectURL(blob);if(cacheId)mediaMemoryUrls.set(cacheId,u)}
      setSrc(u)
    }
    const run=async()=>{
      if(typeof media==='string'){setSrc(media);setLoading(false);return}
      if(media?.dataUrl){setSrc(media.dataUrl);setLoading(false);return}
      if(!id){setSrc('');setLoading(false);return}
      const mem=mediaMemoryUrls.get(id)
      if(mem){setSrc(mem);setLoading(false);return}
      const cached=await getMediaCache(id)
      if(cancelled)return
      if(cached?.blob){setBlob(cached.blob);setLoading(false)}
      const fresh=cached?.savedAt && (Date.now()-cached.savedAt)<MEDIA_CACHE_FRESH_MS
      // True cache-first: do not touch Drive when a fresh local copy exists.
      if(fresh||!enabled||!token){if(!cached?.blob&&!enabled)setLoading(false);return}
      setLoading(!cached?.blob)
      const blob=await fetchDriveMediaBlob(id,token,{retries:!cached?.blob})
      if(cancelled)return
      if(blob){await putMediaCache(id,blob,{name:media?.name,mimeType:media?.mimeType});setBlob(blob)}
      setLoading(false)
    }
    run()
    return()=>{cancelled=true}
  },[typeof media==='object'?media?.driveFileId:media,media?.dataUrl,token,enabled])
  return {src,loading}
}
function MediaImage({media,token,alt='',className='',style,eager=false}){
  const [wrapRef,near]=useNearViewport(eager?'1800px':'500px')
  const {src,loading}=useMediaSrc(media,token,near)
  return <span ref={wrapRef} className={`media-image-shell ${className?`${className}-shell`:''}`}>{src?<img className={className} style={style} src={src} alt={alt} loading={eager?'eager':'lazy'} decoding="async" fetchPriority={eager?'high':'auto'}/>:<span className={`media-placeholder media-loading ${className}`}><FileImage size={24}/><span>{loading?'Loading image…':media?.driveFileId?'Image ready when visible':'No image'}</span></span>}</span>
}
...
function MediaSlider({items=[],token,alt='Image',variant='contain',onRemove=null,eager=false}){
  ...
  return <div className={`media-slider media-slider-${variant}`} onTouchStart={e=>touchStart.current=e.touches?.[0]?.clientX??null} onTouchEnd={endTouch}>
    <div className="media-slider-stage"><MediaImage media={list[index]} token={token} alt={`${alt} ${index+1}`} eager={eager}/></div>
...
async function mediaDataUrl(media,token){
  ...
  const cached=await getMediaCache(media.driveFileId)
  if(cached?.blob)return blobToDataUrl(cached.blob)
  if(!token)return ''
  const blob=await fetchDriveMediaBlob(media.driveFileId,token,{retries:true})
  if(!blob)return ''
  await putMediaCache(media.driveFileId,blob,{name:media.name,mimeType:media.mimeType})
...
function migrate(raw) {
  ...
  merged.draftHistory = (raw.draftHistory || defaultData.draftHistory)
    .map(x => ({ thesisFile:null, ...x }))
    .sort((a,b)=>Number.isFinite(a.displayOrder)&&Number.isFinite(b.displayOrder)?a.displayOrder-b.displayOrder:(b.draft||0)-(a.draft||0))
...
function App() {
  ...
  const [modal,setModal]=useState(null), [editingDraft,setEditingDraft]=useState(null), [toast,setToast]=useState(''), [manualEditActive,setManualEditActive]=useState(()=>localStorage.getItem(LOCAL_UNSAVED_EDITOR_KEY)==='1')
  ...
  const tokenClientRef=useRef(null), autoAuthTriedRef=useRef(false), idleSyncTimerRef=useRef(null), editingRef=useRef(false), syncInFlightRef=useRef(false), dataRef=useRef(data)
  useEffect(()=>{dataRef.current=data;localStorage.setItem(APP_KEY,JSON.stringify(data))},[data])
  useEffect(()=>{ if(toast){const t=setTimeout(()=>setToast(''),3000);return()=>clearTimeout(t)}},[toast])
  useEffect(()=>{document.documentElement.dataset.theme=data.settings.theme},[data.settings.theme])
  useEffect(()=>{
    const isEditor=el=>!!el&&(el.matches?.('input,textarea,select,[contenteditable=\"true\"]')||el.closest?.('[contenteditable=\"true\"]'))
    const onFocusIn=e=>{if(isEditor(e.target))editingRef.current=true}
    const onFocusOut=e=>{if(isEditor(e.target))setTimeout(()=>{const a=document.activeElement;editingRef.current=isEditor(a);if(!editingRef.current)scheduleIdleSync()},250)}
  ...
  },[data.settings.autoSync,drive.token,drive.profile?.email,manualEditActive])
  useEffect(()=>{
    if(!data.settings.autoSync || !drive.token || !drive.profile) return
    const check=()=>{
      const last=Number(localStorage.getItem(LAST_AUTO_SYNC_KEY)||0)
      if(!last || Date.now()-last>=AUTO_SYNC_MS){if(!manualEditActive)syncDrive(drive.token,true)}
    }
    check()
    const timer=setInterval(check,AUTO_SYNC_MS)
    const onVisible=()=>{if(document.visibilityState==='visible'&&!editingRef.current&&!manualEditActive)syncDrive(drive.token,true)}
  ...
  const saveRecoverySnapshot=(reason='before-cloud-pull')=>{try{localStorage.setItem(LOCAL_RECOVERY_KEY,JSON.stringify({reason,at:new Date().toISOString(),data:stripLocalMedia(dataRef.current)}))}catch{}}
  ...
  const scheduleIdleSync=()=>{if(idleSyncTimerRef.current)clearTimeout(idleSyncTimerRef.current);idleSyncTimerRef.current=setTimeout(()=>{if(editingRef.current||localStorage.getItem(LOCAL_UNSAVED_EDITOR_KEY)==='1'){scheduleIdleSync();return}if(data.settings.autoSync&&drive.token&&drive.profile&&!drive.syncing)syncDrive(drive.token,true)},IDLE_SYNC_MS)}
  const update=(key,value)=>{markLocalChanged();setData(d=>{const next={...d,[key]:typeof value==='function'?value(d[key]):value};dataRef.current=next;try{localStorage.setItem(APP_KEY,JSON.stringify(next))}catch{}return next});scheduleIdleSync()}
  const localUpdate=(key,value)=>{localStorage.setItem(LOCAL_UNSAVED_EDITOR_KEY,'1');setManualEditActive(true);setData(d=>{const next={...d,[key]:typeof value==='function'?value(d[key]):value};dataRef.current=next;try{localStorage.setItem(APP_KEY,JSON.stringify(next))}catch{}return next})}
  const commitManualSave=(label='Changes')=>{try{if(idleSyncTimerRef.current)clearTimeout(idleSyncTimerRef.current);localStorage.setItem(APP_KEY,JSON.stringify(dataRef.current));localStorage.removeItem(LOCAL_UNSAVED_EDITOR_KEY);setManualEditActive(false);markLocalChanged();showToast(`${label} saved locally. Safe cloud sync will follow.`);setTimeout(()=>scheduleIdleSync(),900)}catch{showToast('Local save failed.')}}
  const saveLocalNow=(label='Changes')=>{try{if(idleSyncTimerRef.current)clearTimeout(idleSyncTimerRef.current);localStorage.setItem(APP_KEY,JSON.stringify(dataRef.current));showToast(`${label} saved on this device. Cloud sync will run safely after editing.`);setTimeout(()=>scheduleIdleSync(),800)}catch{showToast('Local save failed.')}}
  useEffect(()=>{
  ...
  async function migrateMediaValue(token,value,category,name){
    ...
    let blob=null
    if(value&&typeof value==='object'&&value.localCacheId){const cached=await getMediaCache(value.localCacheId);blob=cached?.blob||null}
    if(!blob&&isDataUrl(dataUrl))blob=dataUrlToBlob(dataUrl)
    ...
    await putMediaCache(uploaded.id,blob,{name:uploaded.name||safeName,mimeType:uploaded.mimeType||blob.type})
  ...
  async function prepareMediaForCloud(token,source=dataRef.current){
    ...
    for(const d of next.draftHistory||[]){if(d.thesisFile)d.thesisFile=await migrateMediaValue(token,d.thesisFile,'thesis',d.thesisFile?.name||`draft-${d.draft||Date.now()}.docx`)}
    next.schemaVersion=SCHEMA_VERSION
    return next
  }
  function stripLocalMedia(source=dataRef.current){
    ...
    for(const d of clean.draftHistory||[]){if(d.thesisFile){if(d.thesisFile.driveFileId)d.thesisFile=strip(d.thesisFile);else d.thesisFile=null}}
    return clean
  }
  ...
  function referencedLearningMediaIds(source=dataRef.current){
  ...
  async function uploadDriveData(token,existing=null,{backup=true}={}){
    ...
    let migrated=await prepareMediaForCloud(token,dataRef.current)
    ...
    dataRef.current=migrated;setData(migrated)
  ...
  async function safeCloudImportAfterAuth(token, quiet=false){
    if(localStorage.getItem(LOCAL_UNSAVED_EDITOR_KEY)==='1'){setManualEditActive(true);if(!quiet)showToast('Local Draft belum disimpan. Cloud pull dihentikan supaya data tidak hilang.');return}
  ...
  async function saveDrive(token=drive.token){
    if(localStorage.getItem(LOCAL_UNSAVED_EDITOR_KEY)==='1'){showToast('Ada Local Draft belum disimpan. Tekan Save dahulu.');return}
    if(token && typeof token !== 'string') token=drive.token
  ...
  async function loadDrive(token=drive.token){
    if(localStorage.getItem(LOCAL_UNSAVED_EDITOR_KEY)==='1'){showToast('Local Draft belum disimpan. Download cloud dibatalkan supaya edit semasa tidak hilang.');return}
    if(token && typeof token !== 'string') token=drive.token
  ...
  async function syncDrive(token=drive.token,quiet=false){
    if(manualEditActive||localStorage.getItem(LOCAL_UNSAVED_EDITOR_KEY)==='1'){if(!quiet)showToast('Ada perubahan belum disimpan. Tekan Save dahulu sebelum sync ke Drive.');return}
    if(token && typeof token !== 'string') token=drive.token
  ...
  const props={data,update,localUpdate,commitManualSave,setModal,showToast,saveLocalNow,setManualEditActive,driveToken:drive.token,uploadLearningAttachment,uploadDraftAttachment}
...
function PenCanvas({value,onSave,token}){
  ...
  useEffect(()=>{let cancelled=false;(async()=>{if(typeof value==='string'){setLoadedValue(value);return}if(value?.dataUrl){setLoadedValue(value.dataUrl);return}if(!value?.driveFileId){setLoadedValue('');return}const v=await mediaDataUrl(value,token);if(!cancelled)setLoadedValue(v||'')})();return()=>{cancelled=true}},[typeof value==='object'?value?.driveFileId:value,value?.dataUrl,token])
...
function LearningPointPage({data,update,localUpdate,commitManualSave,showToast,saveLocalNow,driveToken,uploadLearningAttachment}){
  ...
  const patch=(key,value)=>localUpdate('learningPoints',xs=>xs.map(x=>x.id===selectedId?{...x,[key]:value}:x))
...
function ExpertsPage({data,update,localUpdate,commitManualSave,driveToken,saveLocalNow}){
  ...
  const upload=(id,file)=>{if(!file)return;const reader=new FileReader();reader.onload=()=>localUpdate('experts',xs=>xs.map(i=>i.id===id?{...i,image:pendingMedia(reader.result,file.name,'experts'),cropX:i.cropX??50,cropY:i.cropY??24,cropZoom:i.cropZoom??1}:i));reader.readAsDataURL(file)}
  const patch=(id,key,value)=>localUpdate('experts',xs=>xs.map(i=>i.id===id?{...i,[key]:value}:i))
...
function ConsultationPage({data,update,localUpdate,commitManualSave,driveToken,saveLocalNow}){
  ...
  return <div className="consultation-page"><div className="section-title"><div><h2>SV Consultation</h2><p>Structured consultation notes, action items and visual evidence.</p></div><div className="button-row"><button className="secondary" onClick={printAll}><Printer size={16}/> Print / PDF</button><button className="primary" onClick={add}><Plus/> Add Log</button></div></div><div className="consultation-card-list">{data.consultations.map((x,index)=><div className="card consultation-card" key={x.id}><div className="consultation-card-head"><div><span className="eyebrow">CONSULTATION {String(index+1).padStart(2,'0')}</span><h3>{x.topic||'SV consultation'}</h3></div><div className="consultation-card-actions"><button className="primary consultation-save-btn" type="button" onClick={()=>commitManualSave?.('SV Consultation')}><Save size={15}/> Save</button><button className="icon-btn danger" onClick={()=>update('consultations',xs=>xs.filter(i=>i.id!==x.id))}><Trash2 size={18}/></button></div></div><div className="consultation-top-row"><Field label="Date"><input type="date" value={x.date} onChange={e=>patch(x.id,'date',e.target.value)}/></Field><Field label="Topic"><input value={x.topic} placeholder="Consultation topic" onChange={e=>patch(x.id,'topic',e.target.value)}/></Field><Field label="Status"><select className={statusClass(x.status)} value={x.status} onChange={e=>patch(x.id,'status',e.target.value)}><option>Dalam tindakan</option><option>Tertangguh</option><option>Selesai</option></select></Field></div><div className="consultation-main-row"><Field label="Supervisor Comment"><textarea rows="7" value={x.comment} placeholder="Supervisor comments, corrections and key points..." onChange={e=>patch(x.id,'comment',e.target.value)}/></Field><div className="consultation-photo-box">{(x.images?.length||x.image)?<MediaSlider items={x.images?.length?x.images:[x.image]} token={driveToken} alt="Consultation evidence" variant="cover43" eager={index<3} onRemove={idx=>localUpdate('consultations',xs=>xs.map(i=>{if(i.id!==x.id)return i;const cur=i.images?.length?i.images:(i.image?[i.image]:[]),images=cur.filter((_,n)=>n!==idx);return {...i,images,image:images[0]||''}}))}/>:null}<label className={`consultation-photo-upload ${(x.images?.length||x.image)?'compact-add':'empty'}`}><ImagePlus size={(x.images?.length||x.image)?22:42}/><span>{(x.images?.length||x.image)?'Tambah gambar':'Upload consultation photo'}</span><input type="file" accept="image/*" multiple onChange={e=>upload(x.id,e.target.files)}/></label></div></div><Field label="Action / Follow-up"><textarea rows="4" value={x.action} placeholder="Action items and follow-up..." onChange={e=>patch(x.id,'action',e.target.value)}/></Field></div>)}</div></div>
}


function DraftPage({data,update,localUpdate,commitManualSave,setModal,setEditingDraft,showToast,driveToken}){
  const [previewFile,setPreviewFile]=useState(null),[dragId,setDragId]=useState('')
  const ordered=[...(data.draftHistory||[])].sort((a,b)=>(a.displayOrder??9999)-(b.displayOrder??9999)||(b.draft||0)-(a.draft||0))
  const uploadThesis=async(id,file)=>{if(!file)return;try{const cacheId=`pending-thesis-${id}-${Date.now()}-${Math.random().toString(36).slice(2)}`;await putMediaCache(cacheId,file,{name:file.name,mimeType:file.type});const att=pendingCachedFileMeta(cacheId,file,'thesis');localUpdate('draftHistory',xs=>xs.map(x=>x.id===id?{...x,thesisFile:att}:x));showToast('Fail tesis disimpan pada device. Tekan Save untuk hantar fail + JSON terkini ke Drive.')}catch(e){showToast(e.message||'Gagal menyimpan fail pada device.')}}
  const removeFile=x=>{if(!x.thesisFile)return;if(!confirm(`Buang ${x.thesisFile.name||'fail tesis'} daripada Draft #${String(x.draft).padStart(3,'0')}?`))return;localUpdate('draftHistory',xs=>xs.map(i=>i.id===x.id?{...i,thesisFile:null}:i));showToast('Fail dibuang daripada rekod local. Tekan Save untuk sahkan perubahan.')}
  const saveDraft=()=>commitManualSave?.('Draft 111 Tracker')
  const reorder=(id,dir)=>{const arr=[...ordered],i=arr.findIndex(x=>x.id===id),j=i+dir;if(i<0||j<0||j>=arr.length)return;[arr[i],arr[j]]=[arr[j],arr[i]];const orderMap=new Map(arr.map((x,n)=>[x.id,n]));localUpdate('draftHistory',xs=>xs.map(x=>({...x,displayOrder:orderMap.has(x.id)?orderMap.get(x.id):(x.displayOrder??9999)})));}
  const dropOn=targetId=>{if(!dragId||dragId===targetId){setDragId('');return}const arr=[...ordered],from=arr.findIndex(x=>x.id===dragId),to=arr.findIndex(x=>x.id===targetId);if(from<0||to<0){setDragId('');return}const [moved]=arr.splice(from,1);arr.splice(to,0,moved);const orderMap=new Map(arr.map((x,n)=>[x.id,n]));localUpdate('draftHistory',xs=>xs.map(x=>({...x,displayOrder:orderMap.get(x.id)??x.displayOrder})));setDragId('')}
  const remove=x=>{
    if(!confirm(`Delete Draft #${String(x.draft).padStart(3,'0')} record?`)) return
    const next=data.draftHistory.filter(i=>i.id!==x.id).map((i,n)=>({...i,displayOrder:n}))
    localUpdate('draftHistory',next)
    localUpdate('profile',{...data.profile,currentDraft:Math.max(0,...next.map(i=>i.draft))})
    showToast('Draft dibuang secara local. Tekan Save untuk sahkan.')
  }
  const printDraftHistory=()=>{const rows=ordered.map(x=>`<tr><td><b>#${String(x.draft).padStart(3,'0')}</b></td><td>${escapeHtml(x.date||'')}</td><td>${escapeHtml(x.focus||'')}</td><td>${escapeHtml(String(x.pages||''))}</td><td>${escapeHtml(x.note||'')}</td><td>${escapeHtml(x.thesisFile?.name||'-')}</td></tr>`).join('');printWindow(`<h1>Draft 111 Tracker</h1><div class="meta">Current Draft: ${String(data.profile.currentDraft).padStart(3,'0')} / ${data.profile.draftGoal}</div><table><tr><th>Draft</th><th>Tarikh</th><th>Fokus</th><th>Muka surat</th><th>Catatan</th><th>Fail Tesis</th></tr>${rows}</table>`,'Draft 111 Tracker')}

  const fileCell=x=><div className="draft-file-actions">{x.thesisFile?<><button className="soft-btn" disabled={!x.thesisFile.driveFileId} onClick={()=>x.thesisFile.driveFileId&&setPreviewFile(x.thesisFile)}>{x.thesisFile.driveFileId?<BookOpen size={14}/>:<Save size={14}/>} {x.thesisFile.driveFileId?'View':'Pending Save'}</button><small>{x.thesisFile.name}</small><button className="icon-btn danger draft-file-delete" title="Remove uploaded file" onClick={()=>removeFile(x)}><Trash2 size={14}/></button></>:<label className="soft-btn file-button"><Upload size={14}/> Upload Word<input type="file" accept=".doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={e=>{uploadThesis(x.id,e.target.files?.[0]);e.target.value=''}}/></label>}</div>

  return <div>
    <div className="hero-stat card">
      <Ring value={pct(data.profile.currentDraft,data.profile.draftGoal)} label="Draft 111" size={160}/>
      <div><span className="eyebrow">CURRENT DRAFT</span><h2>{String(data.profile.currentDraft).padStart(3,'0')} / {data.profile.draftGoal}</h2><p>Naik draft secara konsisten setiap minggu dan simpan sejarah perubahan.</p><button className="primary" onClick={()=>{setEditingDraft(null);setModal('draft')}}><Plus/> Naik Draft</button></div>
    </div>
    <div className="card table-card">
      <div className="section-title draft-history-head"><div><h2>Sejarah Draft</h2><p className="muted">Drag baris atau guna ↑ ↓ untuk susun rekod. Upload fail disimpan local dahulu; tekan Save sebelum cloud sync.</p></div><div className="draft-head-actions"><button className="primary" onClick={saveDraft}><Save size={16}/> Save</button><button className="secondary" onClick={printDraftHistory}><Printer size={16}/> Print / PDF</button></div></div>
      <div className="responsive-table draft-history-table"><table><thead><tr><th>Susun</th><th>Draft</th><th>Tarikh</th><th>Fokus</th><th>Muka surat</th><th>Catatan</th><th>Fail Tesis</th><th>Tindakan</th></tr></thead><tbody>{ordered.map((x,idx)=><tr key={x.id} draggable onDragStart={()=>setDragId(x.id)} onDragOver={e=>e.preventDefault()} onDrop={()=>dropOn(x.id)} className={dragId===x.id?'draft-dragging':''}>
        <td><div className="draft-order-controls"><span className="draft-grip" title="Drag to reorder">⋮⋮</span><button className="icon-btn" disabled={idx===0} title="Move up" onClick={()=>reorder(x.id,-1)}>↑</button><button className="icon-btn" disabled={idx===ordered.length-1} title="Move down" onClick={()=>reorder(x.id,1)}>↓</button></div></td>
        <td><b>#{String(x.draft).padStart(3,'0')}</b></td><td>{x.date}</td><td>{x.focus}</td><td>{x.pages}</td><td>{x.note}</td><td>{fileCell(x)}</td>
      ...
      <div className="draft-history-mobile">{ordered.map((x,idx)=><article className="draft-mobile-card" key={`mobile-${x.id}`} draggable onDragStart={()=>setDragId(x.id)} onDragOver={e=>e.preventDefault()} onDrop={()=>dropOn(x.id)}>
        <div className="draft-mobile-head"><div><span className="draft-mobile-kicker">DRAFT</span><strong>#{String(x.draft).padStart(3,'0')}</strong></div><div className="draft-mobile-meta"><span>{formatDate(x.date)}</span><b>{x.pages || 0} muka surat</b></div></div>
        <div className="draft-mobile-order"><button className="secondary" disabled={idx===0} onClick={()=>reorder(x.id,-1)}>↑ Up</button><button className="secondary" disabled={idx===ordered.length-1} onClick={()=>reorder(x.id,1)}>↓ Down</button></div>
        <div className="draft-mobile-section"><span>Fokus</span><p>{x.focus || '—'}</p></div><div className="draft-mobile-section"><span>Catatan</span><p>{x.note || '—'}</p></div><div className="draft-mobile-section draft-mobile-file"><span>Fail Tesis</span>{fileCell(x)}</div>
        <div className="draft-mobile-actions"><button className="secondary" onClick={()=>{setEditingDraft(x);setModal('draft')}}><Edit3 size={15}/> Edit</button><button className="danger-btn" onClick={()=>remove(x)}><Trash2 size={15}/> Delete</button></div>
      </article>)}</div>
      <div className="draft-bottom-save"><button className="primary" onClick={saveDraft}><Save size={16}/> Save Draft Tracker</button></div>
    </div>
