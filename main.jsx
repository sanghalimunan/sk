...
import {
  ...
  Download, Upload, RotateCcw, Search, Lightbulb, Link2, PenTool, Eraser, Printer, FileUp, ExternalLink, Network, GitFork, FileDown
} from 'lucide-react'
import './styles.css'
...
const defaultData = {
  ...
  mindMaps: [
    { id: uid(), title: 'My Mind Map', rootText: 'Main Topic', nodes: [
      { id: 'mm-a', parentId: null, text: 'Research', side: 'left', color: '#7c3aed' },
      { id: 'mm-b', parentId: null, text: 'Writing', side: 'right', color: '#f59e0b' },
      { id: 'mm-c', parentId: 'mm-a', text: 'Literature Review', side: 'left', color: '#8b5cf6' },
      { id: 'mm-d', parentId: 'mm-a', text: 'Methodology', side: 'left', color: '#22c55e' },
      { id: 'mm-e', parentId: 'mm-b', text: 'Chapter Plan', side: 'right', color: '#f97316' },
...
function migrate(raw) {
  ...
  merged.diary = (raw.diary || defaultData.diary).map(d => ({ image:'', labels:[], ...d, labels:Array.isArray(d.labels)?d.labels:[], reflection: d.reflection ?? d.text ?? '' }))
  ...
  merged.mindMaps = (raw.mindMaps || defaultData.mindMaps).map(x => ({ title:'Mind Map', rootText:'Main Topic', nodes:[], updatedAt:'', ...x, nodes:Array.isArray(x.nodes)?x.nodes:[] }))
  return merged
}
...
function App() {
  ...
  async function migrateMediaValue(token,value,category,name){
    ...
    if(typeof value==='object'&&value.driveFileId)return value
    ...
    return {id:value?.id||uid(),driveFileId:uploaded.id,name:uploaded.name||safeName,mimeType:uploaded.mimeType||blob.type,category,dataUrl,pending:false}
  }
  ...
  return <div className={`app ${data.settings.compact?'compact':''}`}>
    ...
    {sidebar&&<div className="sidebar-scrim" onClick={()=>setSidebar(false)}/>}<main><header className="topbar"><button className="icon-btn menu-btn" onClick={()=>setSidebar(true)}><Menu/></button><div className="welcome"><h1>{page==='dashboard'?<>Selamat kembali, <span>{data.profile.name}</span></>:title}</h1><p>{page==='dashboard'?`Anda berada pada Hari ke-${Math.max(1,elapsedDays)} perjalanan PhD GBT anda`:'Urus, pantau dan kemas kini rekod anda.'}</p></div><div className="top-actions"><button className="icon-btn" onClick={()=>update('settings',s=>({...s,theme:s.theme==='dark'?'light':'dark'}))}>{data.settings.theme==='dark'?<Sun/>:<Moon/>}</button>{drive.profile&&<button className="drive-button" onClick={()=>syncDrive()}><Cloud size={18}/><span>{drive.syncing?'Menyegerak...':'Sync Sekarang'}</span></button>}<button className="avatar avatar-sync" type="button" onClick={profileReconnectTestSync} disabled={drive.syncing} title="Sign in / refresh Google Drive session" aria-label="Sign in / refresh Google Drive session"><CircleUserRound/><span>{drive.syncing?'CONNECTING...':(drive.profile?.given_name||'SIGN IN')}</span></button></div></header>
      <section className="content">
        ...
        {page==='calendar'&&<CalendarPage {...props}/>} {page==='learning'&&<LearningPointPage {...props}/>} {page==='mindmap'&&<MindMapPage {...props}/>} {page==='draft'&&<DraftPage {...props} setEditingDraft={setEditingDraft}/>} 
...
function CalendarPage({data,update,setModal,driveToken}){
  ...
  const searchResults=useMemo(()=>{const q=query.trim().toLowerCase();if(!q)return[];const rows=[];data.events.forEach(e=>{if(`${e.title} ${e.date} ${e.start} ${e.end}`.toLowerCase().includes(q))rows.push({id:`event-${e.id}`,type:'Activity',date:e.date,title:e.title,detail:`${e.start}–${e.end}`})});data.diary.forEach(d=>{if(`${d.reflection||''} ${d.win||''} ${(d.labels||[]).join(' ')} ${d.date}`.toLowerCase().includes(q))rows.push({id:`diary-${d.id}`,type:'Diary',date:d.date,title:(d.reflection||'Diary entry').slice(0,90),detail:(d.labels||[]).join(' • ')||d.win||''})});return rows.sort((a,b)=>b.date.localeCompare(a.date)).slice(0,20)},[query,data.events,data.diary])
  ...
  return <div className="calendar-page-wrap">
    <div className="calendar-toolbar">
      <button className={`soft-btn ${view==='timeline'?'active-view':''}`} onClick={()=>setView(v=>v==='timeline'?'calendar':'timeline')}><BarChart3 size={16}/> {view==='timeline'?'Calendar View':'Monthly Timeline'}</button>
      <button className="soft-btn calendar-search-toggle" onClick={()=>{setSearchOpen(v=>!v);if(searchOpen)setQuery('')}}><Search size={16}/> Search</button>
    </div>
    ...
    {view==='timeline'?<MonthlyDiaryTimeline data={data} month={month} setMonth={setMonth} label={timelineLabel} setLabel={setTimelineLabel} labels={allLabels} driveToken={driveToken} jumpTo={jumpTo}/>:<div className="page-grid"><div className="card calendar-full"><div className="calendar-head"><button className="icon-btn" onClick={()=>setMonth(new Date(month.getFullYear(),month.getMonth()-1,1))}><ChevronLeft/></button><h2>{month.toLocaleDateString('ms-MY',{month:'long',year:'numeric'})}</h2><button className="icon-btn" onClick={()=>setMonth(new Date(month.getFullYear(),month.getMonth()+1,1))}><ChevronRight/></button></div><div className="weekdays">{['Isn','Sel','Rab','Kha','Jum','Sab','Aha'].map(x=><b key={x}>{x}</b>)}</div><div className="month-grid">{days.map((d,i)=>d?<button key={i} className={`${iso(d)===selected?'selected':''} ${iso(d)===iso()?'today':''}`} onClick={()=>setSelected(iso(d))}><span>{d.getDate()}</span>{data.events.some(e=>e.date===iso(d))&&<i className="red-dot"/>}{data.diary.some(e=>e.date===iso(d))&&<em className="red-dot second"/>}</button>:<div key={i}/>)}</div></div><div className="card day-panel"><CardTitle title="Calendar & Diary" icon={<CalendarDays/>}/><h2>{formatDate(selected,{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</h2><div className="schedule-list">{selectedEvents.map(e=><div key={e.id} className="schedule red-only"><div><b>{e.start}–{e.end}</b><span>{e.title}</span></div><button className="icon-btn danger" onClick={()=>update('events',xs=>xs.filter(i=>i.id!==e.id))}><Trash2 size={15}/></button></div>)}</div><div className="button-row calendar-actions"><button className="primary" onClick={()=>setModal({type:'event',date:selected})}><Plus/> Activity</button><button className="secondary" onClick={()=>setModal({type:'diary',date:selected})}><Edit3/> {selectedDiary?'Edit Diary':'Write Diary'}</button></div>{selectedDiary?<div className="calendar-diary-detail"><div className="calendar-diary-head"><div><span className="eyebrow">DIARY ENTRY</span><h3>{formatDate(selected,{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</h3>{selectedDiary.labels?.length>0&&<div className="diary-labels">{selectedDiary.labels.map(x=><span key={x}>{x}</span>)}</div>}</div><div className="calendar-diary-head-actions"><button className="soft-btn" onClick={()=>setModal({type:'diary',date:selected})}><Edit3 size={15}/> Edit</button><button className="danger-btn diary-delete-btn" onClick={()=>{if(confirm(`Delete diary entry for ${formatDate(selected,{day:'numeric',month:'long',year:'numeric'})}?`)) update('diary',xs=>xs.filter(i=>i.date!==selected))}}><Trash2 size={15}/> Delete</button></div></div>{selectedDiary.image&&<div className="calendar-diary-image"><MediaImage media={selectedDiary.image} token={driveToken} alt={`Diary ${selected}`}/></div>}<p className="calendar-reflection">{selectedDiary.reflection||'No reflection written.'}</p><div className="calendar-diary-meta"><span>Mood {'●'.repeat(selectedDiary.mood)}{'○'.repeat(5-selectedDiary.mood)}</span></div></div>:<div className="calendar-diary-empty"><BookOpen size={24}/><span>No diary entry saved for this date.</span></div>}<div className="hijri-card"><small>HIJRI</small><strong>{formatHijri(selected)}</strong></div></div></div>}
  </div>
}

function MonthlyDiaryTimeline({data,month,setMonth,label,setLabel,labels,driveToken,jumpTo}){
  const prefix=`${month.getFullYear()}-${String(month.getMonth()+1).padStart(2,'0')}`
  const entries=useMemo(()=>data.diary.filter(d=>d.date?.startsWith(prefix)).filter(d=>label==='ALL'||(d.labels||[]).includes(label)).sort((a,b)=>a.date.localeCompare(b.date)),[data.diary,prefix,label])
  return <div className="card monthly-diary-timeline"><div className="timeline-view-head"><div><span className="eyebrow">MONTHLY DIARY TIMELINE</span><h2>{month.toLocaleDateString('ms-MY',{month:'long',year:'numeric'})}</h2><small>{entries.length} diary entr{entries.length===1?'y':'ies'} displayed</small></div><div className="timeline-view-controls"><button className="icon-btn" onClick={()=>setMonth(new Date(month.getFullYear(),month.getMonth()-1,1))}><ChevronLeft/></button><select value={label} onChange={e=>setLabel(e.target.value)}><option value="ALL">All labels</option>{labels.map(x=><option key={x} value={x}>{x}</option>)}</select><button className="icon-btn" onClick={()=>setMonth(new Date(month.getFullYear(),month.getMonth()+1,1))}><ChevronRight/></button></div></div>{entries.length?<div className="story-timeline">{entries.map((d,i)=><article key={d.id} className={`story-node ${i%2?'right':'left'}`}><button className="story-date" onClick={()=>jumpTo(d.date)}><strong>{new Date(d.date+'T00:00:00').getDate()}</strong><span>{new Date(d.date+'T00:00:00').toLocaleDateString('ms-MY',{weekday:'short'}).toUpperCase()}</span><small>{new Date(d.date+'T00:00:00').toLocaleDateString('ms-MY',{month:'short',year:'numeric'}).toUpperCase()}</small></button><i className="story-dot"/><div className="story-card">{d.labels?.length>0&&<div className="diary-labels">{d.labels.map(x=><span key={x}>{x}</span>)}</div>}<p>{d.reflection||'No reflection written.'}</p>{d.image?<div className="story-image"><MediaImage media={d.image} token={driveToken} alt={`Diary ${d.date}`}/></div>:<div className="story-no-image"><FileImage size={20}/><span>No image</span></div>}<button className="link-btn" onClick={()=>jumpTo(d.date)}>Open diary <ChevronRight size={14}/></button></div></article>)}</div>:<div className="timeline-empty"><BookOpen size={30}/><h3>No diary found</h3><p>Tiada diary untuk bulan atau label yang dipilih.</p></div>}</div>
}
...
function MindMapPage({data,update,showToast}){
  ...
  useEffect(()=>{if(!maps.some(m=>m.id===selectedMapId))setSelectedMapId(maps[0]?.id||null)},[maps,selectedMapId])
  const patchMap=(patch)=>update('mindMaps',xs=>xs.map(m=>m.id===map.id?{...m,...patch,updatedAt:new Date().toISOString()}:m))
  const patchNodes=(fn)=>patchMap({nodes:fn(map.nodes||[])})
  const createMap=()=>{const id=uid();update('mindMaps',xs=>[{id,title:`Mind Map ${xs.length+1}`,rootText:'Main Topic',nodes:[],updatedAt:new Date().toISOString()},...xs]);setSelectedMapId(id);setSelectedNodeId(null)}
  const addBranch=()=>{const id=uid(), count=(map.nodes||[]).filter(n=>!n.parentId).length;patchNodes(ns=>[...ns,{id,parentId:null,text:'New Branch',side:count%2?'right':'left',color:['#7c3aed','#f97316','#16a34a','#0891b2','#e11d48'][count%5]}]);setSelectedNodeId(id)}
  const addChild=()=>{if(!selectedNodeId)return addBranch();const parent=(map.nodes||[]).find(n=>n.id===selectedNodeId);if(!parent)return;const id=uid();patchNodes(ns=>[...ns,{id,parentId:parent.id,text:'New Topic',side:parent.side||'right',color:parent.color||'#e11d48'}]);setSelectedNodeId(id)}
  const removeNode=()=>{if(!selectedNodeId)return;const descendants=new Set([selectedNodeId]);let changed=true;while(changed){changed=false;(map.nodes||[]).forEach(n=>{if(n.parentId&&descendants.has(n.parentId)&&!descendants.has(n.id)){descendants.add(n.id);changed=true}})}patchNodes(ns=>ns.filter(n=>!descendants.has(n.id)));setSelectedNodeId(null)}
  const selected=(map.nodes||[]).find(n=>n.id===selectedNodeId)
  const layout=useMemo(()=>{
    if(!map)return {nodes:[],root:{x:600,y:350},w:1200,h:700}
    const nodes=map.nodes||[], root={x:600,y:350}, out=[]
    const sides=['left','right']
    sides.forEach(side=>{
      const roots=nodes.filter(n=>!n.parentId&&(n.side||'right')===side)
      const branchH=Math.max(100,560/Math.max(1,roots.length))
      roots.forEach((r,ri)=>{
        const baseY=70+branchH*(ri+.5)
        const walk=(node,depth,y,spread)=>{
          const x=root.x+(side==='right'?1:-1)*(180+depth*190)
          out.push({...node,x,y})
          const kids=nodes.filter(n=>n.parentId===node.id)
          if(kids.length){const step=Math.max(52,spread/Math.max(1,kids.length));kids.forEach((k,i)=>walk(k,depth+1,y+(i-(kids.length-1)/2)*step,Math.max(70,spread*.62)))}
        }
        walk(r,0,baseY,Math.min(180,branchH*.8))
      })
    })
    return {nodes:out,root,w:1200,h:700}
  },[map])
  const pos=new Map(layout.nodes.map(n=>[n.id,n]))
  const exportPdf=()=>{const svg=document.querySelector('.mindmap-svg');if(!svg)return;const clone=svg.cloneNode(true);clone.setAttribute('width','1200');clone.setAttribute('height','700');printWindow(`<h1>${escapeHtml(map.title||'Mind Map')}</h1><div class="meta">strategiSK Mindmap Creator</div><div style="width:100%;page-break-inside:avoid">${clone.outerHTML}</div>`,map.title||'Mind Map')}
  if(!map)return <div className="card mindmap-empty"><Network size={44}/><h2>Mindmap Creator</h2><p>Create your first visual mind map.</p><button className="primary" onClick={createMap}><Plus/> New Mind Map</button></div>
  return <div className="mindmap-page">
    <div className="card mindmap-toolbar">
      <div className="mindmap-map-select"><Network size={20}/><select value={map.id} onChange={e=>{setSelectedMapId(e.target.value);setSelectedNodeId(null)}}>{maps.map(m=><option key={m.id} value={m.id}>{m.title}</option>)}</select><button className="secondary" onClick={createMap}><Plus size={16}/> New Map</button></div>
      <div className="mindmap-actions"><button className="secondary" onClick={addBranch}><GitFork size={16}/> Add Branch</button><button className="secondary" onClick={addChild}><Plus size={16}/> Add Child</button><button className="primary" onClick={()=>showToast('Mind map saved. Tekan Sync Sekarang untuk simpan ke Google Drive.')}><Save size={16}/> Save</button><button className="secondary" onClick={exportPdf}><FileDown size={16}/> Export PDF</button></div>
    </div>
    <div className="mindmap-workspace">
      <div className="card mindmap-canvas-card">
        <div className="mindmap-title-edit"><input value={map.title} onChange={e=>patchMap({title:e.target.value})} placeholder="Mind map title"/><small>Tap mana-mana node untuk edit. Susunan cabang dibuat secara automatik.</small></div>
        <div className="mindmap-scroll">
          <svg className="mindmap-svg" viewBox="0 0 1200 700" role="img" aria-label={map.title}>
            <rect width="1200" height="700" rx="28" className="mindmap-bg"/>
            {layout.nodes.map(n=>{const parent=n.parentId?pos.get(n.parentId):layout.root;const sx=parent.x+(n.x>parent.x?70:-70), ex=n.x+(n.x>parent.x?-75:75), mx=(sx+ex)/2;return <path key={`l-${n.id}`} d={`M ${sx} ${parent.y} C ${mx} ${parent.y}, ${mx} ${n.y}, ${ex} ${n.y}`} fill="none" stroke={n.color||'#e11d48'} strokeWidth="4" strokeLinecap="round"/>})}
            <g className="mindmap-root" onClick={()=>setSelectedNodeId(null)}><rect x="510" y="315" width="180" height="70" rx="25"/><text x="600" y="357" textAnchor="middle">{map.rootText||'Main Topic'}</text></g>
            {layout.nodes.map(n=><g key={n.id} className={`mindmap-node ${selectedNodeId===n.id?'selected':''}`} onClick={()=>setSelectedNodeId(n.id)}><rect x={n.x-78} y={n.y-28} width="156" height="56" rx="22" fill={n.color||'#e11d48'} fillOpacity=".14" stroke={n.color||'#e11d48'} strokeWidth={selectedNodeId===n.id?4:2.5}/><text x={n.x} y={n.y+5} textAnchor="middle">{n.text}</text></g>)}
          </svg>
        </div>
      </div>
      <aside className="card mindmap-inspector">
        <span className="eyebrow">NODE EDITOR</span><h3>{selected?'Selected Topic':'Main Topic'}</h3>
        {!selected?<Field label="Central Topic"><input value={map.rootText||''} onChange={e=>patchMap({rootText:e.target.value})}/></Field>:<><Field label="Text"><textarea rows="4" value={selected.text||''} onChange={e=>patchNodes(ns=>ns.map(n=>n.id===selected.id?{...n,text:e.target.value}:n))}/></Field><Field label="Side"><select value={selected.side||'right'} onChange={e=>patchNodes(ns=>ns.map(n=>n.id===selected.id?{...n,side:e.target.value}:n))}><option value="left">Left</option><option value="right">Right</option></select></Field><Field label="Branch Colour"><div className="mindmap-colors">{['#e11d48','#f97316','#eab308','#16a34a','#0891b2','#2563eb','#7c3aed','#db2777'].map(c=><button key={c} className={selected.color===c?'active':''} style={{background:c}} onClick={()=>patchNodes(ns=>ns.map(n=>n.id===selected.id?{...n,color:c}:n))} aria-label={`Colour ${c}`}/>)}</div></Field><div className="button-row"><button className="secondary" onClick={addChild}><Plus size={15}/> Add Child</button><button className="danger-btn" onClick={removeNode}><Trash2 size={15}/> Delete Node</button></div></>}
        <div className="mindmap-tip"><b>Tip</b><p>Gunakan cabang utama untuk tema besar, kemudian Add Child untuk pecahkan idea kepada subtopik. Semua perubahan masuk dalam backup dan Smart Sync strategiSK.</p></div>
        {maps.length>1&&<button className="text-danger mindmap-delete-map" onClick={()=>{if(confirm('Delete this mind map?')){update('mindMaps',xs=>xs.filter(x=>x.id!==map.id));setSelectedNodeId(null)}}}><Trash2 size={14}/> Delete Mind Map</button>}
      </aside>
    </div>
  </div>
}

function SettingsPage({data,update,drive,saveDrive,loadDrive,syncDrive,testDrive,reconnectDrive,exportJson,importJson,manualDriveBackup}){const p=data.profile,setP=(k,v)=>update('profile',{...p,[k]:v});return <div className="settings-grid"><div className="card form-card"><CardTitle title="Profile & GBT Target" icon={<Settings/>}/><Field label="Name"><input value={p.name} onChange={e=>setP('name',e.target.value)}/></Field><Field label="Tagline"><input value={p.tagline} onChange={e=>setP('tagline',e.target.value)}/></Field><div className="form-grid"><Field label="Tarikh mula PhD"><input type="date" value={p.startDate} onChange={e=>setP('startDate',e.target.value)}/></Field><Field label="Target GBT"><input type="date" value={p.targetDate} onChange={e=>setP('targetDate',e.target.value)}/></Field><Field label="Draft semasa"><input type="number" value={p.currentDraft} onChange={e=>setP('currentDraft',+e.target.value)}/></Field><Field label="Sasaran draft"><input type="number" value={p.draftGoal} onChange={e=>setP('draftGoal',+e.target.value)}/></Field></div></div><div className="card form-card"><CardTitle title="Personal Google Drive" icon={<Cloud/>}/><div className={`connection ${drive.profile?'connected':''}`}><Cloud size={28}/><div><b>{drive.profile?`Disambung: ${drive.profile.email}`:'Belum disambung'}</b><small>{drive.lastSync?`Sync terakhir ${drive.lastSync}`:'Sambungkan akaun Google yang sama pada PC dan telefon.'}</small></div></div><div className="drive-diagnostics"><div><span>Scopes appData + media</span><b className={drive.scopeOk?'diag-ok':'diag-warn'}>{drive.scopeOk?'OK':'Belum disahkan'}</b></div><div><span>Google Drive API</span><b className={drive.apiOk?'diag-ok':'diag-warn'}>{drive.apiOk?'OK':'Belum diuji'}</b></div><div><span>Fail cloud</span><b>{drive.fileStatus}</b></div>{drive.lastError&&<div className="diag-error"><span>Ralat terakhir</span><b>{drive.lastError}</b></div>}</div><div className="button-row"><button className="primary" onClick={()=>syncDrive()}><Cloud/> Smart Sync</button><button className="secondary" onClick={()=>testDrive()}><Database/> Uji Drive</button><button className="secondary" onClick={()=>reconnectDrive()}><RotateCcw/> Reconnect</button><button className="secondary" onClick={()=>saveDrive()}><CloudUpload/> Paksa Simpan</button><button className="secondary" onClick={()=>loadDrive()}><CloudDownload/> Paksa Muat Turun</button></div><p className="hint"><b>Uji Drive</b> semak data tersembunyi dan folder media. v32.1 menggunakan <code>drive.appdata</code> untuk database serta <code>drive.file</code> untuk folder <b>My Drive → strategiSK</b>. Smart Sync: jika device berubah, perubahan dihantar ke Drive; jika device tidak berubah, data terbaru Drive dimuat turun. Selepas naik taraf, tambah kedua-dua scope dalam Google Auth Platform → Data Access dan tekan <b>Reconnect</b> sekali. Origin semasa: <code>{window.location.origin}</code></p></div><div className="card form-card storage-card"><CardTitle title="Safe Storage Architecture — v32.1" icon={<Database/>}/><p className="hint">Database teks kekal di <b>appDataFolder</b>. Gambar dan sketch dipindahkan semasa sync ke folder biasa dalam My Drive.</p><div className="storage-tree"><code>strategiSK/</code><code>├─ diary/</code><code>├─ learning-point/</code><code>├─ learning-files/</code><code>├─ consultation/</code><code>├─ experts/</code><code>├─ sketches/</code><code>└─ backups/</code></div><p className="hint">Schema data semasa: <b>v{data.schemaVersion||SCHEMA_VERSION}</b>. Data versi lama dimigrate tanpa reset.</p></div><div className="card form-card"><CardTitle title="FOW & FOD Recipients" icon={<Send/>}/><Field label="Nombor WhatsApp SV / CRMP (format 6012...)"><input value={p.whatsapp} onChange={e=>setP('whatsapp',e.target.value)} placeholder="60123456789"/></Field><Field label="Telegram Chat ID"><input value={p.telegramChatId} onChange={e=>setP('telegramChatId',e.target.value)} placeholder="123456789"/></Field><p className="hint">Butang hantar terletak terus dalam bahagian FOW dan FOD.</p></div><div className="card form-card"><CardTitle title="Display & Backup" icon={<Download/>}/><label className="check-card"><input type="checkbox" checked={data.settings.compact} onChange={e=>update('settings',{...data.settings,compact:e.target.checked})}/><span><b>Compact dashboard</b><small>Kurangkan jarak kad untuk skrin kecil.</small></span></label><label className="check-card"><input type="checkbox" checked={data.settings.showGraphics} onChange={e=>update('settings',{...data.settings,showGraphics:e.target.checked})}/><span><b>Elemen grafik</b><small>Papar chart dan ilustrasi.</small></span></label><label className="check-card"><input type="checkbox" checked={data.settings.autoSync!==false} onChange={e=>update('settings',{...data.settings,autoSync:e.target.checked})}/><span><b>Auto Sync setiap 6 jam</b><small>Apabila Google Drive telah disambungkan dan app sedang dibuka, strategiSK akan menjalankan Smart Sync secara automatik setiap 6 jam: upload hanya jika device berubah, selain itu refresh dari Drive.</small></span></label><div className="button-row"><button className="secondary" onClick={exportJson}><Download/> Export JSON</button><button className="secondary" onClick={()=>manualDriveBackup()}><CloudUpload/> Snapshot Drive</button><label className="secondary file-button"><Upload/> Import JSON<input type="file" accept="application/json" onChange={importJson}/></label><button className="danger-btn" onClick={()=>{if(confirm('Reset semua data?')){localStorage.removeItem(APP_KEY);location.reload()}}}><RotateCcw/> Reset</button></div></div></div>}
...
function DiaryModal({data,update,close,initialDate=iso(),driveToken}){
  const makeEntry=(date)=>data.diary.find(d=>d.date===date)||{id:uid(),date,reflection:'',win:'',mood:4,image:'',labels:[]}
  const[x,setX]=useState({image:'',labels:[],...makeEntry(initialDate)}),[labelText,setLabelText]=useState('')
  const commonLabels=['Writing','Reading','Supervisor','Methodology','Data','Analysis','Publication','Milestone','Personal']
  const upload=file=>{if(!file)return;const reader=new FileReader();reader.onload=()=>setX(v=>({...v,image:pendingMedia(reader.result,file.name,'diary')}));reader.readAsDataURL(file)}
  ...
  return <Modal title="Daily Diary / TDR" onClose={close}><Field label="Tarikh"><input type="date" value={x.date} onChange={e=>{const next=e.target.value;setX({image:'',labels:[],...makeEntry(next)})}}/></Field><Field label="Refleksi harian"><textarea rows="7" value={x.reflection} onChange={e=>setX({...x,reflection:e.target.value})}/></Field><Field label="Win kecil hari ini"><input value={x.win} onChange={e=>setX({...x,win:e.target.value})}/></Field><Field label="Diary Labels"><div className="label-entry"><input value={labelText} onChange={e=>setLabelText(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();addLabel(labelText)}}} placeholder="Contoh: Writing, Supervisor, Milestone"/><button className="secondary" type="button" onClick={()=>addLabel(labelText)}><Plus size={14}/> Add</button></div><div className="label-quick">{commonLabels.map(lbl=><button type="button" key={lbl} className={(x.labels||[]).includes(lbl)?'selected':''} onClick={()=> (x.labels||[]).includes(lbl)?setX(v=>({...v,labels:v.labels.filter(i=>i!==lbl)})):addLabel(lbl)}>{lbl}</button>)}</div>{x.labels?.length>0&&<div className="diary-labels editable">{x.labels.map(lbl=><button type="button" key={lbl} onClick={()=>setX(v=>({...v,labels:v.labels.filter(i=>i!==lbl)}))}>{lbl}<X size={12}/></button>)}</div>}</Field><Field label={`Mood / tenaga: ${x.mood}/5`}><input type="range" min="1" max="5" value={x.mood} onChange={e=>setX({...x,mood:+e.target.value})}/></Field><Field label="Gambar diari / bukti progress"><label className={`diary-upload ${x.image?'has-image':''}`}>{x.image?<MediaImage media={x.image} token={driveToken} alt="Preview diari"/>:<ImagePlus size={34}/>}<span>{x.image?'Klik untuk tukar gambar':'Upload gambar'}</span><input type="file" accept="image/*" onChange={e=>upload(e.target.files?.[0])}/></label>{x.image&&<button className="text-danger" onClick={()=>setX({...x,image:''})}><Trash2 size={14}/> Buang gambar</button>}</Field><button className="primary full" onClick={()=>{update('diary',xs=>[...xs.filter(i=>i.date!==x.date),{...x,labels:x.labels||[]}]);close()}}><Save/> Simpan Diari</button></Modal>
}
