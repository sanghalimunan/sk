...
function MonthlyDiaryTimeline({data,month,setMonth,label,setLabel,labels,driveToken,jumpTo}){
  ...
  return <div className="card monthly-diary-timeline"><div className="timeline-view-head"><div><span className="eyebrow">MONTHLY TIMELINE</span><h2>{month.toLocaleDateString('ms-MY',{month:'long',year:'numeric'})}</h2><small>{entries.length} item{entries.length===1?'':'s'} displayed</small></div><div className="timeline-view-controls"><button className="icon-btn" onClick={()=>setMonth(new Date(month.getFullYear(),month.getMonth()-1,1))}><ChevronLeft/></button><select value={label} onChange={e=>setLabel(e.target.value)}><option value="ALL">All items</option><option value="Activity">Activity</option>{labels.map(x=><option key={x} value={x}>{x}</option>)}</select><button className="icon-btn" onClick={()=>setMonth(new Date(month.getFullYear(),month.getMonth()+1,1))}><ChevronRight/></button></div></div>{dayGroups.length?<div className="story-timeline day-group-timeline">{dayGroups.map((g,i)=>{
