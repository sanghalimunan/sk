...
function App() {
  ...
  async function profileReconnectTestSync(){
    ...
    showToast('Menyambung semula Google Drive...')
    const afterAuth=async(token)=>{
      ...
      try{
        ...
        setDrive(x=>({...x,apiOk:true,fileStatus:existing?`Data + folder media OK`:'Drive API OK — data belum wujud',lastError:''}))
        showToast('Drive OK. Menyegerakkan data terkini...')
        await syncDrive(token,false)
      }catch(e){
        setDrive(x=>({...x,syncing:false,lastError:e.message,fileStatus:'Ujian Drive gagal'}))
        showToast(`Sync dihentikan: ${e.message}`)
      }
    }
    try{
