...
function App() {
  ...
  const localUpdate=(key,value)=>{localStorage.setItem(LOCAL_UNSAVED_EDITOR_KEY,'1');setManualEditActive(true);setData(d=>{const next={...d,[key]:typeof value==='function'?value(d[key]):value};try{localStorage.setItem(APP_KEY,JSON.stringify(next))}catch{}return next})}
  const commitManualSave=(label='Changes')=>{try{if(idleSyncTimerRef.current)clearTimeout(idleSyncTimerRef.current);localStorage.setItem(APP_KEY,JSON.stringify(data));localStorage.removeItem(LOCAL_UNSAVED_EDITOR_KEY);setManualEditActive(false);markLocalChanged();showToast(`${label} saved locally. Safe cloud sync will follow.`);setTimeout(()=>scheduleIdleSync(),900)}catch{showToast('Local save failed.')}}
