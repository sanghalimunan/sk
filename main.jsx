...
const defaultData = {
  ...
  learningPoints: [
    { id: uid(), date: iso(), title: 'Learning Point 01', notes: 'Catat apa yang dipelajari, idea, rumusan bacaan atau perkara penting di sini.', notesHtml:'', images: [], attachments: [], links: [], prompts: [], sketch: '' },
  ],
...
function migrate(raw) {
  ...
  merged.learningPoints = (raw.learningPoints || defaultData.learningPoints).map(x => ({ images:[], attachments:[], links:[], prompts:[], sketch:'', notesHtml:'', ...x }))
...
function LearningPointPage({data,update,showToast,driveToken,uploadLearningAttachment}){
  ...
  const add=()=>{const item={id:uid(),date:iso(),title:`Learning Point ${String(list.length+1).padStart(2,'0')}`,notes:'',notesHtml:'',images:[],attachments:[],links:[],prompts:[],sketch:''};update('learningPoints',xs=>[item,...xs]);setSelectedId(item.id)}
...
function DraftPage({data,update,setModal,setEditingDraft}){
  ...
  const printDraftHistory=()=>{const rows=[...(data.draftHistory||[])].sort((a,b)=>b.draft-a.draft).map(x=>`<tr><td><b>#${String(x.draft).padStart(3,'0')}</b></td><td>${escapeHtml(x.date||'')}</td><td>${escapeHtml(x.focus||'')}</td><td>${escapeHtml(String(x.pages||''))}</td><td>${escapeHtml(x.note||'')}</td></tr>`).join('');printWindow(`<h1>Draft 111 Tracker</h1><div class="meta">Current Draft: ${String(data.profile.currentDraft).padStart(3,'0')} / ${data.profile.draftGoal}</div><table><tr><th>Draft</th><th>Tarikh</th><th>Fokus</th><th>Muka surat</th><th>Catatan</th></tr>${rows}</table>`,'Draft 111 Tracker')}
