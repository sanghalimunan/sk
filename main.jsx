...
import {
  ...
  Download, Upload, RotateCcw, Search, Lightbulb, Link2, PenTool, Eraser
} from 'lucide-react'
import './styles.css'
...
const defaultData = {
  ...
  learningPoints: [
    { id: uid(), date: iso(), title: 'Learning Point 01', notes: 'Catat apa yang dipelajari, idea, rumusan bacaan atau perkara penting di sini.', images: [], links: [], prompts: [], sketch: '' },
  ],
...
function migrate(raw) {
  ...
  merged.learningPoints = (raw.learningPoints || defaultData.learningPoints).map(x => ({ images:[], links:[], prompts:[], sketch:'', ...x }))
  return merged
}
...
function App() {
  ...
  function stripLocalMedia(source){
    ...
    for(const lp of clean.learningPoints||[]){lp.images=(lp.images||[]).map(strip);lp.sketch=strip(lp.sketch)}
    return clean
  }
...
function LearningPointPage({data,update,showToast,driveToken}){
  const list=data.learningPoints||[],[selectedId,setSelectedId]=useState(list[0]?.id||null)
  ...
  const add=()=>{const item={id:uid(),date:iso(),title:`Learning Point ${String(list.length+1).padStart(2,'0')}`,notes:'',images:[],links:[],prompts:[],sketch:''};update('learningPoints',xs=>[item,...xs]);setSelectedId(item.id)}
