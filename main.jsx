...
import {
  ...
  Download, Upload, RotateCcw, Search, Lightbulb, Link2, PenTool, Eraser, Printer, FileUp, ExternalLink, Folder, FolderOpen
} from 'lucide-react'
import './styles.css'
...
function ConsultationPage({data,update,driveToken}){
  ...
  const printAll=async()=>{const tab=window.open('','_blank','width=1100,height=800');if(!tab)return;tab.document.write('<p style="font-family:Arial;padding:30px">Preparing SV Consultation…</p>');const rows=[];for(const x of [...data.consultations].sort((a,b)=>b.date.localeCompare(a.date))){const img=await mediaDataUrl(x.image,driveToken);rows.push(`<div class="section" style="page-break-inside:avoid"><h2>${escapeHtml(x.topic||'SV Consultation')}</h2><div class="meta">${escapeHtml(formatDate(x.date))} • ${escapeHtml(x.status||'')}</div><p><b>Supervisor Comment</b><br>${nl2br(x.comment||'')}</p><p><b>Action / Follow-up</b><br>${nl2br(x.action||'')}</p>${img?`<img class="print-img" style="max-width:520px" src="${img}">`:''}</div>`)}printWindow(`<h1>SV Consultation</h1>${rows.join('')}`,'SV Consultation',tab)}
