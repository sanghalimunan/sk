...
function App() {
  ...
  useEffect(()=>{
    ...
    const onVisible=()=>{if(document.visibilityState==='visible'&&!editingRef.current)syncDrive(drive.token,true)}
  ...
  const props={data,update,setModal,showToast,saveLocalNow,driveToken:drive.token,uploadLearningAttachment,uploadDraftAttachment}
...
function LearningPointPage({data,update,showToast,saveLocalNow,driveToken,uploadLearningAttachment}){
  ...
  const uploadImages=async files=>{saveLocalNow?.('Learning Point');const arr=Array.from(files||[]);if(!arr.length)return;try{const imgs=await Promise.all(arr.map(async f=>pendingMedia(await readImageFile(f),f.name,'learning')));patch('images',[...(current.images||[]),...imgs.filter(Boolean)]);showToast(`${imgs.length} image(s) added. Media will move to Google Drive on next sync.`)}catch(e){showToast(e.message)}}
...
function ExpertsPage({data,update,driveToken,saveLocalNow}){
  ...
  const upload=(id,file)=>{saveLocalNow?.('Expert List');if(!file)return;const reader=new FileReader();reader.onload=()=>update('experts',xs=>xs.map(i=>i.id===id?{...i,image:pendingMedia(reader.result,file.name,'experts'),cropX:i.cropX??50,cropY:i.cropY??24,cropZoom:i.cropZoom??1}:i));reader.readAsDataURL(file)}
