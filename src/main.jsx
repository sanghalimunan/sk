...
function LearningPointPage({data,update,showToast}){
  ...
  const uploadImages=async files=>{const arr=Array.from(files||[]);if(!arr.length)return;try{const imgs=await Promise.all(arr.map(f=>readImageFile(f)));patch('images',[...(current.images||[]),...imgs.filter(Boolean)]);showToast(`${imgs.length} image(s) added.`)}catch(e){showToast(e.message)}}
...
function DiaryModal({data,update,close,initialDate=iso()}){const makeEntry=(date)=>data.diary.find(d=>d.date===date)||{id:uid(),date,reflection:'',win:'',mood:4,image:''};const[x,setX]=useState({image:'',...makeEntry(initialDate)});const upload=file=>{if(!file)return;const reader=new FileReader();reader.onload=()=>setX(v=>({...v,image:reader.result}));reader.readAsDataURL(file)};return <Modal title="Daily Diary / TDR" onClose={close}><Field label="Tarikh"><input type="date" value={x.date} onChange={e=>{const next=e.target.value;setX({image:'',...makeEntry(next)})}}/></Field><Field label="Refleksi harian"><textarea rows="7" value={x.reflection} onChange={e=>setX({...x,reflection:e.target.value})}/></Field><Field label="Win kecil hari ini"><input value={x.win} onChange={e=>setX({...x,win:e.target.value})}/></Field><Field label={`Mood / tenaga: ${x.mood}/5`}><input type="range" min="1" max="5" value={x.mood} onChange={e=>setX({...x,mood:+e.target.value})}/></Field><Field label="Gambar diari / bukti progress"><label className={`diary-upload ${x.image?'has-image':''}`}>{x.image?<img src={x.image} alt="Preview diari"/>:<ImagePlus size={34}/>}<span>{x.image?'Klik untuk tukar gambar':'Upload gambar'}</span><input type="file" accept="image/*" onChange={e=>upload(e.target.files?.[0])}/></label>{x.image&&<button className="text-danger" onClick={()=>setX({...x,image:''})}><Trash2 size={14}/> Buang gambar</button>}</Field><button className="primary full" onClick={()=>{update('diary',xs=>[...xs.filter(i=>i.date!==x.date),x]);close()}}><Save/> Simpan Diari</button></Modal>}
