...
const formatHijri = (date) => { const d = new Date(date + 'T00:00:00'); try { return new Intl.DateTimeFormat('ms-MY-u-ca-islamic-umalqura',{day:'numeric',month:'long',year:'numeric'}).format(d) } catch { try { return new Intl.DateTimeFormat('ms-MY-u-ca-islamic',{day:'numeric',month:'long',year:'numeric'}).format(d) } catch { return '' } } }
...
function App() {
  ...
  async function loadDrive(token=drive.token){if(token && typeof token !== 'string') token=drive.token;if(!token)return initGoogleClient(loadDrive,true);setDrive(x=>({...x,syncing:true}));try{const existing=await findDriveFile(token);if(!existing)throw new Error('Drive API OK tetapi fail strategiSK belum wujud. Buat Sync Dua Hala dari PC dahulu atau tekan Paksa Simpan.');const remote=await fetchRemoteData(token,existing),savedAt=remote.meta?.savedAt||existing.modifiedTime;try{await createVisibleBackup(token,data,'before-force-download')}catch{} delete remote.meta;setData(migrate(remote));localStorage.setItem(LOCAL_UPDATED_KEY,savedAt||new Date().toISOString());setDrive(x=>({...x,fileId:existing.id,syncing:false,lastSync:new Date().toLocaleTimeString('ms-MY',{hour:'2-digit',minute:'2-digit'}),apiOk:true,fileStatus:`Fail ditemui: ${DRIVE_FILE}`,lastError:''}));showToast('Data Google Drive berjaya dimuatkan ke peranti ini.')}catch(e){setDrive(x=>({...x,syncing:false,lastError:e.message}));showToast(e.message)}}
  async function syncDrive(token=drive.token){
    ...
    try{
      ...
      if(!localTime||remoteTime>localTime){const savedAt=remote.meta?.savedAt||existing.modifiedTime;if(localTime){try{await createVisibleBackup(token,data,'before-cloud-pull')}catch{}} delete remote.meta;setData(migrate(remote));localStorage.setItem(LOCAL_UPDATED_KEY,savedAt);showToast('Data terbaru dari Google Drive dimuatkan.')}
...
function CalendarPage({data,update,setModal,driveToken}){
  const [selected,setSelected]=useState(iso()),[month,setMonth]=useState(new Date()),[query,setQuery]=useState(''),[searchOpen,setSearchOpen]=useState(false)
...
function DiaryModal({data,update,close,initialDate=iso(),driveToken}){const makeEntry=(date)=>data.diary.find(d=>d.date===date)||{id:uid(),date,reflection:'',win:'',mood:4,image:''};const[x,setX]=useState({image:'',...makeEntry(initialDate)});const upload=file=>{if(!file)return;const reader=new FileReader();reader.onload=()=>setX(v=>({...v,image:pendingMedia(reader.result,file.name,'diary')}));reader.readAsDataURL(file)};return <Modal title="Daily Diary / TDR" onClose={close}><Field label="Tarikh"><input type="date" value={x.date} onChange={e=>{const next=e.target.value;setX({image:'',...makeEntry(next)})}}/></Field><Field label="Refleksi harian"><textarea rows="7" value={x.reflection} onChange={e=>setX({...x,reflection:e.target.value})}/></Field><Field label="Win kecil hari ini"><input value={x.win} onChange={e=>setX({...x,win:e.target.value})}/></Field><Field label={`Mood / tenaga: ${x.mood}/5`}><input type="range" min="1" max="5" value={x.mood} onChange={e=>setX({...x,mood:+e.target.value})}/></Field><Field label="Gambar diari / bukti progress"><label className={`diary-upload ${x.image?'has-image':''}`}>{x.image?<MediaImage media={x.image} token={driveToken} alt="Preview diari"/>:<ImagePlus size={34}/>}<span>{x.image?'Klik untuk tukar gambar':'Upload gambar'}</span><input type="file" accept="image/*" onChange={e=>upload(e.target.files?.[0])}/></label>{x.image&&<button className="text-danger" onClick={()=>setX({...x,image:''})}><Trash2 size={14}/> Buang gambar</button>}</Field><button className="primary full" onClick={()=>{update('diary',xs=>[...xs.filter(i=>i.date!==x.date),x]);close()}}><Save/> Simpan Diari</button></Modal>}
