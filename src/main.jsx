  ...
  const units=useMemo(()=>{
    ...
    for(const item of entries){
      if(item._type==='activity'){
        let group=out.find(x=>x._type==='activity-group'&&x.date===item.date)
        if(!group){group={_type:'activity-group',date:item.date,items:[],_sort:item._sort};out.push(group)}
        group.items.push(item)
      }else out.push(item)
    }
    return out.sort((a,b)=>(a._sort||`${a.date} 00:00`).localeCompare(b._sort||`${b.date} 00:00`))
  },[entries])
  return <div className="card monthly-diary-timeline"><div className="timeline-view-head"><div><span className="eyebrow">MONTHLY TIMELINE</span><h2>{month.toLocaleDateString('ms-MY',{month:'long',year:'numeric'})}</h2><small>{entries.length} item{entries.length===1?'':'s'} displayed</small></div><div className="timeline-view-controls"><button className="icon-btn" onClick={()=>setMonth(new Date(month.getFullYear(),month.getMonth()-1,1))}><ChevronLeft/></button><select value={label} onChange={e=>setLabel(e.target.value)}><option value="ALL">All items</option><option value="Activity">Activity</option>{labels.map(x=><option key={x} value={x}>{x}</option>)}</select><button className="icon-btn" onClick={()=>setMonth(new Date(month.getFullYear(),month.getMonth()+1,1))}><ChevronRight/></button></div></div>{units.length?<div className="story-timeline">{units.map((d,i)=>{
    const date=d.date
    return <article key={d._type==='activity-group'?`activity-${date}`:`diary-${d.id}`} className={`story-node ${i%2?'right':'left'} ${d._type==='activity-group'?'activity-node':''}`}><button className="story-date" onClick={()=>jumpTo(date)}><strong>{new Date(date+'T00:00:00').getDate()}</strong><span>{new Date(date+'T00:00:00').toLocaleDateString('ms-MY',{weekday:'short'}).toUpperCase()}</span><small>{new Date(date+'T00:00:00').toLocaleDateString('ms-MY',{month:'short',year:'numeric'}).toUpperCase()}</small></button><i className="story-dot"/>{d._type==='activity-group'?<div className="activity-timeline-grid">{d.items.map(a=><div key={a.id} className="activity-story-card compact-activity-card"><div className="activity-badge">ACTIVITY</div><h3>{a.title}</h3><p><b>{a.start}–{a.end}</b>{a.done?' • Completed':''}</p><button className="link-btn" onClick={()=>jumpTo(a.date)}>Open <ChevronRight size={13}/></button></div>)}</div>:<div className="story-card">{d.labels?.length>0&&<div className="diary-labels">{d.labels.map(x=><span key={x}>{x}</span>)}</div>}<p>{d.reflection||'No reflection written.'}</p>{d.image?<div className="story-image"><MediaImage media={d.image} token={driveToken} alt={`Diary ${d.date}`}/></div>:<div className="story-no-image"><FileImage size={20}/><span>No image</span></div>}<button className="link-btn" onClick={()=>jumpTo(d.date)}>Open date <ChevronRight size={14}/></button></div>}</article>})}</div>:<div className="timeline-empty"><BookOpen size={30}/><h3>No timeline item found</h3><p>Tiada diary atau activity untuk bulan / label yang dipilih.</p></div>}</div>
}
